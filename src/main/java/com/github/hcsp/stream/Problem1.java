...
public class Problem1 {
    ...
    public static int countUsers(List<User> users) {
        return (int) users.stream().filter(user -> user.age >= 60 && 2 == user.name.length()).count();
    }
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        ...
        users.stream().filter(user -> user.age >= 60).sorted(new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                return Integer.compare(o2.age, o1.age);
            }
        }).forEach(user -> res.addLast(user.name));
        return res;
    }
