...
public class Problem1 {
    ...
    public static int countUsers(List<User> users) {
        return (int) users.stream()
                .filter(Problem1::getUserAgeOver60AndWithNameLength2)
                .count();
    }

    private static boolean getUserAgeOver60AndWithNameLength2(User user) {
        return user.getAge() >= 60 && user.getName().length() == 2;
    }
