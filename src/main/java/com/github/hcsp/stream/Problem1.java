...
public class Problem1 {
    ...
    public static int countUsers(List<User> users) {
        return (int) users.stream()
                .filter(user -> user.age >= 60 && user.name.length() == 2)
                .count();
    }
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        LinkedList<String> userList = users.stream()
                .filter(user -> user.age >= 60)
                .sorted(Comparator.comparing((User user) -> user.age).reversed())//发现一个坑 这里User必须指定
                .map(user -> user.name)
