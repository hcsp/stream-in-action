...
public class Problem1 {
    ...
    static class User {
        ...
        private boolean isElderUser() {
            return this.getAge() >= 60;
        }

        private boolean isElderAndNameLengthUser() {
            return getAge() >= 60 && getName() != null && getName().length() == 2;
        }
    }

    // 编写一个方法，统计"年龄大于等于60的用户中，名字是两个字的用户数量"
    public static int countUsers(List<User> users) {
        return Math.toIntExact(users.stream().filter(User::isElderAndNameLengthUser).count());
    }
