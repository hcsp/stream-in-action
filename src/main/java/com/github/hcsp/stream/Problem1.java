...
public class Problem1 {
    ...
    public static int countUsers(List<User> users) {
        return users.stream().filter(x -> x.age >= 60 && x.name.length() == 2).toArray().length;
    }
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        ...
        return users.stream().filter(x -> x.age >= 60)
                .sorted(Comparator.comparingInt(x -> -x.age)).map(x -> x.name)
