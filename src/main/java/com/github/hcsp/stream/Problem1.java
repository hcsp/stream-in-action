...
public class Problem1 {
    static class User {
        ...
        public boolean isAgeGe60() {
            return age >= 60;
        }

        public boolean isNameEq2() {
            return name.length() == 2;
        }

        public boolean isAgeGe60AndNameLengthEq2() {
            return isAgeGe60() && isNameEq2();
        }
    }

    // 编写一个方法，统计"年龄大于等于60的用户中，名字是两个字的用户数量"
    public static int countUsers(List<User> users) {
        return (int) users.stream().filter(User::isAgeGe60AndNameLengthEq2).count();
    }
