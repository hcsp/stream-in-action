...
public class Problem1 {
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        ...
        return users.stream().filter(user -> user.age >= 60).sorted(ComparableAge()).map(user -> user.name).collect(Collectors.toCollection(LinkedList::new));
    }

    private static Comparator<User> ComparableAge() {
        return (o1, o2) -> {
            if (o1.age > o2.age) {
                return -1;
