...
public class Problem1 {
    ...
    public static int countUsers(List<User> users) {
        return (int) users.stream().filter(s -> s.age >= 60).filter(s -> s.name.length() == 2).count();
    }
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        Comparator<User> compare = Comparator.comparing(User::getAge).reversed();
        return users.stream().filter(user -> user.age >= 60).sorted(compare).map(User::getName).collect(
