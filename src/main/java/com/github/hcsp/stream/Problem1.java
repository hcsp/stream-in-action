...
public class Problem1 {
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        return users.stream().filter(user -> user.age >= 60).sorted(Problem1::compare).map(user -> user.name).collect(Collectors.toCollection(LinkedList::new));
    }

    private static int compare(User u1, User u2) {
        if (u1.age != u2.age) {
            return Integer.compare(u1.age, u2.age) * -1;
        } else {
