...
public class Problem1 {
    ...
    public static int countUsers(List<User> users) {
        return Long.valueOf(users.stream().filter(u -> u.age >= 60).filter(u -> u.name.length() == 2).count()).intValue();
    }
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        return users.stream().filter(u -> u.age >= 60).sorted(Comparator.<User>comparingInt(u -> u.age).reversed()).map(u -> u.name)
