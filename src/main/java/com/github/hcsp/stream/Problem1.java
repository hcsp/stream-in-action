...
public class Problem1 {
    ...
    public static int countUsers(List<User> users) {
        return (int) users.stream().filter(a -> a.name.length() <= 2).filter(b -> b.age >= 60).count();
    }
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        return users.stream().filter(a -> a.age >= 60)
                .sorted(Comparator.comparing(a -> ((User) a).age).reversed()).map(user -> user.name).collect(Collectors.toCollection(LinkedList::new));
    }
