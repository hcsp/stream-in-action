...
public class Problem1 {
    ...
    public static int countUsers(List<User> users) {
        return (int) users.stream().filter(user -> (user.getAge() >= 60))
                .filter(user -> (user.getName().length() == 2)).count();
    }
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        return users.stream().filter(user -> (user.getAge() >= 60))
                .sorted(Comparator.comparing(User::getAge)).map(User::getName)
