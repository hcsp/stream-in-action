...
public class Problem1 {
    ...
    public static int countUsers(List<User> users) {
        return users.stream().filter(user -> user.age >= 60 && user.name.length() ==2).toArray().length;
    }
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        return users.stream().filter(user -> user.age >= 60)
                .sorted(Comparator.comparing(User::getAge).reversed()).map(user -> user.name).collect(Collectors.toCollection(LinkedList::new));
    }
