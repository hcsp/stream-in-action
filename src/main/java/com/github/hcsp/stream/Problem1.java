...
public class Problem1 {
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        return users.stream().filter(user -> user.age >= 60).sorted(Comparator.comparingInt(User::getAge).reversed()).map(user -> user.name).collect(Collectors.toCollection(LinkedList::new));
    }
