...
public class Problem1 {
    ...
    private static boolean AgeEqualAndMoreThanSixty(User user) {
        return user.age >= 60;
    }
    ...
    public static int countUsers(List<User> users) {
        return (int) users.stream()
                .filter(user -> user.getName().length() == 2 && AgeEqualAndMoreThanSixty(user))
                .count();
    }
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        return users.stream()
                .filter(Problem1::AgeEqualAndMoreThanSixty)
