...
public class Problem1 {
    ...
    public static int countUsers(List<User> users) {
        return Long.valueOf(users.stream().filter(user -> user.age >= 60 && user.name.length() == 2).count()).intValue();
    }
    ...
    public static LinkedList<String> collectNames(List<User> users) {
        return users.stream().filter(user -> user.age >= 60).sorted(((o1, o2) -> o2.age - o1.age)).map(user -> user.name).collect(Collectors.toCollection(LinkedList::new));
    }
