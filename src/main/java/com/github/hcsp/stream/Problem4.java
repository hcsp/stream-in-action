...
public class Problem4 {
    ...
    public static Map<String, List<Employee>> collect(List<Employee> employees) {
        return employees.stream().collect(Collectors.toMap(Employee::getDepartment, employee -> {
            List<Employee> list = new ArrayList<>();
            list.add(employee);
            return list;
        }, (List<Employee> ep1, List<Employee> ep2) -> {
            ep1.addAll(ep2);
            List<Employee> tempEmployee = ep1.stream().sorted(Comparator.comparing(Employee::getAge)).collect(Collectors.toList());
            return tempEmployee;
        }));
    }
